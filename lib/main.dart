...
Future<void> main() async {
  ...
  await BackendService.ensureAnonymousSession();
  final session = await AppSession.load();
  runApp(PrimeYardApp(initialSession: session));
}

class FirebaseConfig {
  static const options = FirebaseOptions(
    apiKey: 'AIzaSyCBOobo6kK3Yq92NSglZfHVKGm0wGW1gps',
    appId: '1:266595410249:web:297defb5f7c188015a7a98',
    messagingSenderId: '266595410249',
    projectId: 'primeyard-workspace',
    storageBucket: 'primeyard-workspace.firebasestorage.app',
    authDomain: 'primeyard-workspace.firebaseapp.com',
  );
}
...
class PrimeYardApp extends StatefulWidget {
  ...
  const PrimeYardApp({super.key, required this.initialSession});
...
class BackendService {
  ...
  static Future<void> ensureDefaultAdmin() async {
    ...
    if (current.users.isNotEmpty) return;
    final admin = {
      'id': 'admin',
      'username': 'admin',
      'displayName': 'Owner',
      'role': 'master_admin',
      'passwordHash': _hash('PrimeYard2025'),
      'createdAt': _today(),
    };
    final seed = {
      ...current.toMap(),
      ...
      'updatedBy': 'flutter_v3_seed',
    };
    await _doc.set(seed, SetOptions(merge: true));
    await _cacheStateMap({
      ...current.toMap(),
      'users': [admin],
    });
    await _cacheUsers([admin]);
  }
  ...
  static Future<void> saveState(WorkspaceState state, {String updatedBy = 'flutter_v3'}) async {
    final data = {
      ...state.toMap(),
      'updatedAt': FieldValue.serverTimestamp(),
      'updatedBy': updatedBy,
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _boot() async {
    await BackendService.ensureDefaultAdmin();
            ...
            actions: [
              IconButton(
                onPressed: () => showAboutDialog(
                  ...
                  children: const [Text('Native Flutter rebuild with Firebase sync.')],
                ),
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    state.updatedAt == null ? 'Live sync ready' : 'Last sync ${DateFormat('HH:mm').format(state.updatedAt!)}',
